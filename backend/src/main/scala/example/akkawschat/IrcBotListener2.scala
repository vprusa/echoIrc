...
class IrcBotListener2(var chat: Chat, var server: String, var channel: String) extends ListenerAdapter {

  val logger: Logger = LoggerFactory.getLogger(this.getClass.getName)

  val ECHO_PATTERN = Pattern.compile("(?i)echo[ ]+(.+)")

  def IrcBotListener(chat: Chat, server: String, channel: String) {
    this.chat = chat
    this.server = server
