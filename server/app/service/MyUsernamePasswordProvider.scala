...
class MyUsernamePasswordProvider[U](
  ...
  override val InvalidCredentials = "securesocial.login.invalidCredentials"

  override def authenticateForApi(implicit request: Request[AnyContent]): Future[AuthenticationResult] = {
  ...
  private def profileForCredentials(userId: String, password: String): Future[Option[BasicProfile]] = {
    Logger.debug("MyUsernamePasswordProvider profileForCredentials")
    userService.find(id, userId).map { maybeUser => {
      Logger.debug("MyUsernamePasswordProvider profileForCredentials maybeUser ")
      Logger.debug(maybeUser.toString)
      for (
        user <- maybeUser;
        pinfo <- user.passwordInfo;
        hasher <- passwordHashers.get(pinfo.hasher) if hasher.matches(pinfo, password)
      ) yield {
        Logger.debug("MyUsernamePasswordProvider profileForCredentials user")
  ...
  override protected def authenticationFailedResult[A](apiMode: Boolean)(implicit request: Request[A]) = Future.successful {
    if (apiMode)
  ...
  override protected def withUpdatedAvatar(profile: BasicProfile): Future[BasicProfile] = {
