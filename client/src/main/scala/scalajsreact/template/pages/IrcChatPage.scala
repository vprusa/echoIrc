...
object IrcChatPage {
  ...
  class Backend($: BackendScope[String, State]) {
    ...
    def sendMessage(ws: WebSocket, msg: String): Callback = {
      ...
      def send = Callback(ws.send(s"""{"sender":"userBot","message":"userBot"}"""))
    ...
    def start: Callback = {
      ...
      connect.attemptTry.flatMap {
        case Success(ws)    => $.modState(_.log(s"Connecting to ${url}").copy(ws = Some(ws)))
  ...
  val WebSocketsApp = ScalaComponent.builder[String]("WebSocketsApp")
